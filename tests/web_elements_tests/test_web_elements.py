...
def test_calendar(driver, base_page, web_page):
    ...
    test_date = (12, 20, 2024)
    day = test_date[1]
    month = test_date[0]
    year = test_date[2]

    expected_date = f"{month}/{day}/{year}"
    # expected_date = "12/20/2024"

    web_page.set_calendar_date(*test_date)
    # web_page.set_calendar_date(2000, 3, 15)

    actual_date = driver.find_element(*web_page.calendar_selector).get_attribute("value")

    assert actual_date == expected_date
    print(f"Ожидаемая дата: {expected_date}\nФактическая дата в календаре: {actual_date}")


def test_select_dropdown(driver, base_page, web_page):
    driver.get(web_page.dropdown_url)
    print(f"Открывам url: {web_page.dropdown_url}")

    driver.find_element(web_page.select_dropdown).click()
    print("Клик по дропдауну")
